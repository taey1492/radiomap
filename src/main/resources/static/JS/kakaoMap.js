...
const marker_earthquake = "../img/marker_earthquake.png"; // 지진 마커
...
const eqYearPush = async () => {
    ...
    for (let i = 0; i < eqJson.length; i++) {
        ...
        let eqAntemeri = eqJson[i].antemeri; // AM, PM -> 오전, 오후로 변경
        if (eqAntemeri == "AM") {
            eqAntemeri = "오전";
        } else {
            eqAntemeri = "오후";
        ...
        // 마커 이미지
        const markerImage = new kakao.maps.MarkerImage(marker_earthquake, imageSize, imageOption);
